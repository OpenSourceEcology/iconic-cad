...
function renderInspector() {
  ...
  ui['selected-detail'].textContent = entry ? `Catalog bounds: ${entry.bounds_mm.join(' × ')} mm. Geometry ${entry.validation.geometry}; engineering review pending.` : 'Select a placed machine to inspect its catalog bounds.';
...
ui['duplicate-instance'].addEventListener('click', () => { const original = instanceFor(selectedId); if (!original) return; workspace = addMachineInstance(workspace, original.entry_id, [original.position_mm[0] + 150, original.position_mm[1] + 150, original.position_mm[2]]); const duplicate = workspace.instances.at(-1); workspace.instances.at(-1).rotation_deg = original.rotation_deg; selectedId = duplicate.id; refresh(); message('Duplicated selected machine with a 150 mm offset.', 'success'); });
...
ui['export-bom'].addEventListener('click', () => { if (!workspace.instances.length) return message('Place a machine before exporting a BOM.', 'error'); download(bomCsv(workspace, catalog), `${fileStem()}-bom.csv`, 'text/csv'); message('BOM CSV exported.', 'success'); });
...
renderer.domElement.addEventListener('pointerup', event => { if (!pointerStart || Math.hypot(event.clientX - pointerStart[0], event.clientY - pointerStart[1]) > 4) return; const rect = renderer.domElement.getBoundingClientRect(); pointer.set(((event.clientX - rect.left) / rect.width) * 2 - 1, -((event.clientY - rect.top) / rect.height) * 2 + 1); raycaster.setFromCamera(pointer, camera); const hit = raycaster.intersectObjects(assembly.children, true)[0]; if (hit?.object.userData.instanceId) { selectedId = hit.object.userData.instanceId; renderInspector(); message('Selected instance.', 'success'); } });
