...
import { addMachineInstance, bomCsv, bomRows, newMachineWorkspace, validateCatalog, validateMachineWorkspace, validateMeshPayload, workspaceFromDemo } from './machine-core.js';
...
async function redrawAssembly(fitWhenReady = false) {
  ...
  if (token === buildToken && fitWhenReady) fitAssembly();
}
...
function renderCatalog() {
  ...
  for (const demo of catalog.demos) {
    ...
    button.addEventListener('click', () => { workspace = workspaceFromDemo(demo, catalog); selectedId = workspace.instances[0]?.id || null; refresh({ fit: true }); message(`Loaded ${demo.title}.`, 'success'); }); ui['demo-list'].append(button);
  }
  for (const entry of catalog.entries) {
    ...
    button.addEventListener('click', () => { workspace = addMachineInstance(workspace, entry.id); selectedId = workspace.instances.at(-1).id; refresh({ fit: true }); message(`${entry.title} placed at the origin.`, 'success'); }); ui['catalog-list'].append(button);
  }
}
